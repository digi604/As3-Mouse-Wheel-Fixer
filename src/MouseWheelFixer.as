package {
	 ...
	 * Copyright (c) 2012 Patrick Lauber
	 *
    ...
    SWFWheel.retrieveObject = function(id) {
		...
		public static function initialize(stage : Stage) : void {
			if (!available || isReady) {
				trace('ERROR: external interface not ready');
				return;
			}
		 ...
		 */
		private static function triggerMouseEvent(deltaX : Number, deltaY : Number, ctrlKey : Boolean, altKey : Boolean, shiftKey : Boolean) : void {
			...
			if (!target) return;
			// create event.
			var event : MouseWheelEvent = new MouseWheelEvent(MouseWheelEvent.MOUSE_WHEEL, true, false, target.mouseX, target.mouseY, null, ctrlKey, altKey, shiftKey, false, int(deltaX), int(deltaY));
			// trigger event.
